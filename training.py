...
def main(config):
    ...
    if write_tensorboard_data:
        ...
        # Always add the first undenoised image to 'pred' so that it's easy to see progression
        raw = data_raw[0]
        plt.imsave('pred.png', raw)
        pred = plt.imread('pred.png')
        writer.add_image('pred', pred, 0, dataformats='HWC')
